...
class LSTMForecaster(nn.Module):
    ...
    def __init__(self, input_size=1, hidden_size=64, num_layers=2, dropout=0.2):
        ...
        self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout if num_layers > 1 else 0, batch_first=True)
...
class LSTMTrainer:
    ...
    def save_model(self, path='models/saved/lstm_model.pt'):
...
def main():
    ...
    pred_df = pd.DataFrame({'timestamp': test.index[seq_length:], 'actual': test['trip_count'].values[seq_length:], 'pred_lstm': predictions})
    pred_df.to_csv('results/lstm_predictions.csv', index=False)
    print(f'✓ Saved predictions to results/lstm_predictions.csv')
    print('\n' + '=' * 60)
