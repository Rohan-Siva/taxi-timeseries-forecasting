...
def evaluate_forecast(y_true, y_pred, model_name='Model'):
    metrics = {'model': model_name, 'MAE': mean_absolute_error(y_true, y_pred), 'RMSE': root_mean_squared_error(y_true, y_pred), 'MAPE': mean_absolute_percentage_error(y_true, y_pred), 'SMAPE': symmetric_mean_absolute_percentage_error(y_true, y_pred), 'R2': r2_score(y_true, y_pred)}
    return metrics
...
def evaluate_all_models(predictions_dict, y_true):
    ...
    for model_name, y_pred in predictions_dict.items():
        ...
        metrics = evaluate_forecast(y_true[:min_len], y_pred[:min_len], model_name=model_name)
...
def print_metrics_table(metrics_df):
    ...
    for _, row in metrics_df.iterrows():
        print(f"{row['model']:<20} {row['MAE']:>10.2f} {row['RMSE']:>10.2f} {row['MAPE']:>9.2f}% {row['SMAPE']:>9.2f}% {row['R2']:>10.4f}")
    print('=' * 80)
